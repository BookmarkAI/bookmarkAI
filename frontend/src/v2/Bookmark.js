    ...
    return(
        ...
        <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between',border:1, borderRadius: 1, borderColor: '#dddddd', p: 0.5}}>
            <Box onClick={()=>setViewer(url)} sx={{display: 'flex', flexDirection: 'row'}}>
                ...
                <Box>
                    <Typography sx={{width: '100%', fontSize: mobile ? 15: 11, overflow: 'hidden', textOverflow: 'ellipsis',  display: '-webkit-box',
                    overflow: 'hidden',
                    WebkitBoxOrient: 'vertical',
                    WebkitLineClamp: 2}}>{title}</Typography>
                    <Typography sx={{fontSize: mobile ? 13 : 11}}>{displayUrl(url)}</Typography>
                </Box>
            </Box>
